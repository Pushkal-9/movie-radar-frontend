...
const MovieSeatLayout = () => {
  ...
  const [seats, setSeats] = useState(Array.from({ length: 12 }, (_, i) => {
    return Array.from({ length: 15 }, (_, j) => {
      return { id: j, isAvailable: true, isBlocked: false,isSelected: false, rate: 0, seatId: -1, row: '', column : -1 };
    });
  }));
        ...
        <Modal open={openModal} class="react-responsive-modal-closeButton">
          <div className="booking-popup">
            ...
            <ul>
              {
                (() => {
                  ...
                  for (let i = 0; i < selectedSeats.length; i++) {
                    ...
                    seatItems.push(<li key={seat.id}>  Seat  {seat.row}{seat.column} :  ${seat.rate}</li>);
                  }
            ...
            <div className="booking-buttons">
              ...
              <button className="ui button primary" onClick={() => console.log('Confirming seats...')}>Confirm Seats</button>
            </div>
          </div>
        </Modal>
      )}
